create table if not exists paymentsense_core.company_sic_codes (
  code text primary key,
  description text not null
);

create table if not exists paymentsense_core.business_types (
  id bigserial primary key,
  name text not null,
  normalized_name text not null,
  sic_code text null references paymentsense_core.company_sic_codes(code) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint uq_business_types_normalized_name unique (normalized_name)
);

create index if not exists ix_business_types_sic_code
  on paymentsense_core.business_types (sic_code);

insert into paymentsense_core.company_sic_codes (code, description)
values
